...
- マイコン固有の関数は `_mcu_wrap_func()` のような`_mcu_wrap_`を付けたラッパーを通して関数を呼び出しすること

- Windows環境固有の関数は `_win_wrap_func()` のような`_win_wrap_`を付けたラッパーを通して関数を呼び出しすること

- Linux環境固有の関数は `_linux_wrap_func()` のような`_linux_wrap_`を付けたラッパーを通して関数を呼び出しすること
